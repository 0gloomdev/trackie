...
import 'package:drift/drift.dart' as drift_;
...
class _ItemDetailScreenState extends ConsumerState<ItemDetailScreen> {
  ...
  void _saveProgress(LearningItem item) {
    ...
    ref
        .read(learningItemsNotifierProvider.notifier)
    ...
    return settingsAsync.when(
      data: (settings) => itemsAsync.when(
        data: (items) {
          ...
          final LearningItem item = items.firstWhere(
                          ...
                          _NotesCard(
                            ...
                            onChanged: (value) {
                              ref
                                  .read(learningItemsNotifierProvider.notifier)
                                  .updateItem(
                                    item.copyWith(notes: drift_.Value(value)),
                                  );
                            },
                          ),
                          const SizedBox(height: 24),
