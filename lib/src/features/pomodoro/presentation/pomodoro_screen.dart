...
class _PomodoroScreenState extends ConsumerState<PomodoroScreen> {
  ...
  void _completeSession() {
    ...
    final sessionList = sessions.maybeWhen(
      data: (list) => list,
      orElse: () {
    ...
    if (sessionList.isNotEmpty) {
      ref
          .read(pomodoroSessionsNotifierProvider.notifier)
          .completeSession(sessionList.last.id);
    }
    ref.read(pomodoroTimeProvider.notifier).state = _selectedMinutes * 60;
    ref.read(userProfileNotifierProvider.notifier).addXp(10);
  ...
  Widget build(BuildContext context) {
    ...
    final todaySessions = sessions.maybeWhen(
      data: (sessionList) {
        final now = DateTime.now();
        final today = DateTime(now.year, now.month, now.day);
        return sessionList
            .where((s) => s.completed && s.startedAt.isAfter(today))
            .length;
      },
      orElse: () => 0,
    );
